...
export default function TrendsScreen() {
  ...
  const refresh = useCallback(async () => {
    ...
    setCategories(await getExpenseCategoryTotalsForMonth(db, month));
    setMonthly(await getMonthlyTotals(db, { monthsBack: 6, endMonthInclusive: month }));
    if (selectedCategoryId) {
      setSubcategories(await getExpenseSubcategoryTotalsForMonth(db, month, selectedCategoryId));
    } else {
      setSubcategories([]);
    }
  ...
  const pieData = useMemo(
    () => categories.map((c, idx) => ({ value: c.totalCents, color: palette[idx % palette.length] })),
    [categories]
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardSubtitle}>本月支出: ¥{centsToYuan(totalExpense)}</Text>
          ...
          {monthly.length === 0 ? (
            ...
            <>
              ...
              <View style={styles.monthLabels}>
                {monthly.map((m) => (
                  <Text key={m.month} style={styles.monthTick}>{formatMonthCN(m.month)}</Text>
                ))}
              </View>
        ...
        {selectedCategoryId && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardSubtitle}>{selectedCategoryName} · ¥{centsToYuan(subTotal)}</Text>
            {subcategories.length === 0 ? (
              <Text style={styles.emptyText}>该分类暂无子分类记录</Text>
            ) : (
              <View style={styles.subList}>
                {subcategories.map((s, idx) => {
                  const pct = subTotal > 0 ? Math.round((s.totalCents / subTotal) * 100) : 0;
                  return (
                    <View key={s.subcategoryId ?? 'none'} style={styles.subBarRow}>
                      <View style={styles.subBarHeader}>
                        <Text style={styles.subName}>{s.subcategoryName ?? '(未分类)'}</Text>
                        <Text style={styles.subAmount}>¥{centsToYuan(s.totalCents)} · {pct}%</Text>
                      </View>
                      <View style={styles.subBarTrack}>
                        <View style={[styles.subBarFill, { width: `${pct}%`, backgroundColor: palette[idx % palette.length] }]} />
                      </View>
                    </View>
                  );
                })}
...
const styles = StyleSheet.create({
  ...
  scrollContent: { flex: 1, backgroundColor: '#F5F5F5', paddingTop: 16 },
  monthRow: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 24, marginBottom: 16 },
  monthBtn: { padding: 8 },
  monthLabel: { fontSize: 14, fontWeight: '600', color: TEXT_PRIMARY },
  card: { backgroundColor: '#FFFFFF', marginHorizontal: 16, marginBottom: 14, borderRadius: 4, padding: 16, gap: 8 },
  cardTitle: { fontSize: 14, fontWeight: '600', color: TEXT_PRIMARY },
  cardSubtitle: { fontSize: 11, color: TEXT_SECONDARY },
  emptyState: { alignItems: 'center', paddingVertical: 32, gap: 12, backgroundColor: 'transparent' },
  emptyText: { fontSize: 13, color: TEXT_SECONDARY },
  pieRow: { flexDirection: 'row', gap: 16, alignItems: 'center', marginTop: 8, backgroundColor: 'transparent' },
  pieLegend: { flex: 1, gap: 8, backgroundColor: 'transparent' },
  legendRow: { flexDirection: 'row', gap: 8, alignItems: 'center', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 3, backgroundColor: '#F5F5F5' },
  legendRowActive: { backgroundColor: `${PRIMARY_GREEN}20`, borderWidth: 1, borderColor: PRIMARY_GREEN },
  swatch: { width: 12, height: 12, borderRadius: 6 },
  legendText: { flex: 1, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: 'transparent' },
  legendName: { fontSize: 13, fontWeight: '500', color: TEXT_PRIMARY },
  legendMeta: { fontSize: 13, color: TEXT_SECONDARY },
  monthLabels: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8, backgroundColor: 'transparent' },
  monthTick: { fontSize: 10, color: TEXT_SECONDARY },
  legendIndicators: { flexDirection: 'row', justifyContent: 'center', gap: 24, marginTop: 12, backgroundColor: 'transparent' },
  legendIndicator: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'transparent' },
  legendDot: { width: 8, height: 8, borderRadius: 4 },
  legendIndicatorText: { fontSize: 12, color: TEXT_SECONDARY },
  subList: { gap: 8, marginTop: 8, backgroundColor: 'transparent' },
  subName: { fontSize: 13, color: TEXT_PRIMARY },
  subAmount: { fontSize: 13, fontWeight: '600', color: TEXT_PRIMARY, fontVariant: ['tabular-nums'] },
  subBarRow: { marginBottom: 12, backgroundColor: 'transparent' },
  subBarHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6, backgroundColor: 'transparent' },
  subBarTrack: { height: 8, borderRadius: 4, backgroundColor: '#F0F0F0', overflow: 'hidden' },
  subBarFill: { height: 8, borderRadius: 4 },
  tapHint: { fontSize: 11, color: PRIMARY_GREEN, marginTop: 10 },
});
